...
class ProxyServer:
    ...
    async def _handle_client(
        ...
        try:
            ...
            # Route pywb.proxy magic hostname directly to pywb backends
            parsed_url = urlparse(url)
            if parsed_url.hostname == "pywb.proxy":
                pywb_backends = self.backend.get_pywb_proxy_backends()
                if pywb_backends:
                    for pb in pywb_backends:
                        response = await pb.forward_raw(url)
                        if response and response.status_code < 400:
                            await self._send_raw_response(writer, response)
                            return
                    # All returned errors — send the last response as-is
                    if response:
                        await self._send_raw_response(writer, response)
                    else:
                        await self._send_error(writer, 502, "Bad Gateway", url=url)
                else:
    ...
    async def _send_raw_response(
        self, writer: asyncio.StreamWriter, response
    ):
        """Send a backend response directly — no transform, cache, header bar, or throttle."""
        reason = self.HTTP_REASONS.get(response.status_code, "OK")
