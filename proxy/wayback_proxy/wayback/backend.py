...
class BackendChain(Backend):
    ...
    def get_pywb_proxy_backends(self) -> list:
        """Return all pywb-proxy backends."""
        from .pywb_client import PywbClient
        return [b for b in self._backends if isinstance(b, PywbClient) and b.mode == "proxy"]
    ...
    for entry in chain_cfg:
        ...
        if btype == "cache":
            ...
            backends.append(PywbClient(
                ...
                target_date=config.wayback.target_date,
                date_tolerance_days=config.wayback.date_tolerance_days,
                mode=entry.get("mode", "replay"),
            ))
        elif btype == "local":
            backends.append(LocalClient(
                base_url=entry.get("base_url", "http://localhost:9000"),
